...
if __name__ == '__main__':
    ...
    for run in tqdm(range(0, args.validation_run, 100)):
        result_dict = pd.read_pickle(f'corrected_validation/validation_dumping_dict_{args.date}/validation_stats_run_{run}.pkl')
    ...
    print(result_df.shape, result_df['variances'].nunique(), result_df['means'].nunique(), result_df['deepFMpredictions'].nunique())
    ...
    os.makedirs(f'corrected_validation/validation_dumping_dict_{args.date}', exist_ok=True)

    with open(f'corrected_validation/validation_dumping_dict_{args.date}/valid_metrics_{date}_{matrix_strategy}', 'wb') as handle:
