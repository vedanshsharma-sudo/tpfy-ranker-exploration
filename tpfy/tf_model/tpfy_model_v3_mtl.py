...
class TpfyMtlModelConfig:
    ...
    enable_discover_popularity: bool
    enable_hp_feature: bool


class TpfyModelV3(tf.keras.Model):
    def __init__(
        self, hparams: TpfyMtlModelConfig, click_ns, enable_random_watch, **kwargs
    ):
        ...
        self.enable_random_watch = enable_random_watch
        ...
        self.user_slots = [
            ...
            schema.is_paid_user,
            *(
                [schema.is_honeypot_user, schema.is_honeypot_enabled]
                if hparams.enable_hp_feature
                else []
            ),
            schema.joined_bucket,
            *(user_identity_slots if not NO_USER_IDENTITY else []),
            *([schema.request_country] if hparams.multi_country else []),
        ...
        if hparams.enable_discover_popularity:
            self.sparse_feature_indices.extend(
                [
                    ScalarIndex.primaryLangPopularityDV,
                    ScalarIndex.primaryLangPopularityWV,
                ]
            )
        ...
        self.linear_layer = Linear(
            output_units=2, l2_reg=self.dnn_l2, use_bias=True, name="linear"
        )

        self.mtl_linear_layer = Linear(
            output_units=2, l2_reg=self.dnn_l2, use_bias=True, name="mtl_linear"
        )
    ...
    def build(self, input_shape):
        ...
        self.click_biases = self.add_weight(
            "click_biases",
            shape=(2,),
            dtype=tf.float32,
            initializer=tf.initializers.zeros(),
        ...
        self.watch_biases = self.add_weight(
            "watch_biases",
            shape=(2,),
            dtype=tf.float32,
            initializer=tf.initializers.zeros(),
    ...
    def call(self, inputs, training=False, compact=False):
        ...
        with tf.name_scope("deepfm"):
            ...
            linear_input = tf.identity(compress_output, name="linear_input")

            target_output = self.linear_layer(linear_input)
            mtl_target_output = self.mtl_linear_layer(linear_input)
            self.log("target_output shape", target_output.shape)

            click_biases = tf.gather(self.click_biases, task)
            watch_biases = tf.gather(self.watch_biases, task)
            if self.enable_random_watch:
                click = target_output[:, 0:1] + click_biases
                watch = target_output[:, 1:2] + watch_biases
            else:
                click = target_output[:, 0:1]
                watch = target_output[:, 1:2]
            paywall_view = mtl_target_output[:, 0:1]
            add_watchlist = mtl_target_output[:, 1:2]

            click_recab = click + math.log(self.click_ns)
            random_watch = tf.sigmoid(click) * tf.sigmoid(watch)

            if compact:
                click = tf.sigmoid(click)
                click_recab = tf.sigmoid(click_recab)
                watch = tf.sigmoid(watch)
                paywall_view = tf.sigmoid(paywall_view)
                add_watchlist = tf.sigmoid(add_watchlist)

            return {
                "random_watch": random_watch,
                "click": click,
                "click_recab": click_recab,
                "watch": watch,
